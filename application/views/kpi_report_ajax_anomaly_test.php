        ...
        ?>
                ...
                foreach($row_kpi_driven as $row){
					...
					if(!empty($row_KPIMetrics)){
						...
						/*foreach($matricsfilter as $row1){
							$co++;
				//			echo $row1['ValueDriver'];
							echo $co;
						}*/
						if(count($matricsfilter) > 0)
						{
							//print_r($matricsfilter[$jj]['ValueDriver']);
						
							if($bus == $row_bus['Business']){
								continue;
							}else{
								$bus = $row_bus['Business'];
							}
            ...
            <div class="row">
                ...
                foreach($row_kpi_driven as $row){
                    ...
                    foreach($matricsfilter as $row1){ $count++; ?>
                        <div class="col-md-4" >
                            <div onclick="doLoadChart('<?php echo $row_bus['Business']; ?>','<?php echo $row['ValueDriver']; ?>','<?php echo str_replace("&","___", $row1['KPIMetrics']); ?>')" aria-hidden="true" style="cursor:grab; width: 95%; font-size:16px; background-color: #ffffc2; color: #444;  margin-top: 10px; margin-bottom: 10px; padding-left: 5px;">
                                <div style="width: 100%;">							
                                    <div style="width: 80%; text-align: left;" >
                                        <?php echo $row1['ValueDriver']; ?>&nbsp;<i class="fa fa-plus" style="font-size:16px; color:#888;"></i>
                                    </div>
                                </div>							
                            </div>
