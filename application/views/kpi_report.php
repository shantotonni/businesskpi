...
<html lang="en">
        ...
        <script>
            ...
            $(document).ready(function(){
                ...
                $("#submit_data").click(function(){
                    ...
                    $.ajax({  
                        type: "POST",  
                        url: "<?php echo base_url(); ?>site/kpi_report_ajax",  
                        data: dataString,
                        beforeSend: function() 
                        {
                            $( ".inner" ).append( "<div id='ReportBDloading'></div>" );            
                            jQuery("#ReportBD").remove();           
                            jQuery("#ReportBDloading").html('<img src="<?php echo base_url(); ?>assets/img/loading.gif" align="absmiddle" width="200" height="100" alt="Loading..."> Loading...');
                        },  
                        success: function(response)
                        {
                ...
                $("#submit_data_anomaly").click(function(){
                    var Business = jQuery("select#Business").val();
                    var charttype = jQuery("select#charttype").val();
                    var CYEAR =  $.map($('input[name="CYEAR"]:checked'), function(c){return c.value; });
                    var KPIMetrics = jQuery("select#KPIMetrics").val();
                    ...
                    $.ajax({  
                        type: "POST",  
                        url: "<?php echo base_url(); ?>site/kpi_report_ajax_anomaly",  
                        data: dataString,
                        beforeSend: function() 
                        {
                            $( ".inner" ).append( "<div id='ReportBDloading'></div>" );            
    ...
    <body>
        ...
        <div class="container-fluid">
            <div class="row">            
                <div class="container-fluid">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-1">
                            <button id="submit_data_anomaly" type="button" class="btn btn-primary">Anomaly</button>
                        </div>
						<div class="col-sm-1">
                            <a  class="btn btn-warning" href="<?php echo base_url('goodbad/kpi_good'); ?>">Good/Bad</a>
                        </div>
                        <div class="col-sm-1">
                            <a target="_blank" class="btn btn-info" href="<?php echo base_url(); ?>compare" target="_blank">Compare Report</a>
                            <a  class="btn btn-info" href="http://192.168.101.176:8050/" target="_blank" style="display: none;">Correlation And Anomaly Detection</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="ReportBD">
                <?php include('kpi_report_ajax.php');?>
            </div>
        </div>
