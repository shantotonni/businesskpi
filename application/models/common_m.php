...
class Common_m extends CI_Model {
    ...
    function SelectCountVW_KPIDETAILSDATA($Business, $year, $KPIMetrics = '') {
    ...
    function SelectKPIBusiness($Business,  $year, $KPIMetrics = '') {
        $CYEAR = myBusinessId('CYEAR' ,$year);
        ($Business == 'All') ? $Business_sql = '' :  $Business_sql = "AND Business = '$Business'";
		if(empty($KPIMetrics)) { $KPIMetrics_sql = ''; }else{   $KPIMetrics_sql = "AND KPIMetrics = '$KPIMetrics'";		}
        $sql = "SELECT DISTINCT Business FROM VW_KPIDETAILSDATA2   WHERE  ($CYEAR)   $Business_sql $KPIMetrics_sql ORDER BY Business";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
    ...
    function SelectKPIDriven($Business, $year , $KPIMetrics = '') {
        $CYEAR = myBusinessId('CYEAR' ,$year);
        ($Business == 'All') ? $Business_sql = '' :  $Business_sql = "AND Business = '$Business'";
		if(empty($KPIMetrics)) { $KPIMetrics_sql = ''; }else{   $KPIMetrics_sql = "AND KPIMetrics = '$KPIMetrics'";		}
        $sql = "SELECT DISTINCT ValueDriver, Business FROM VW_KPIDETAILSDATA2   WHERE  ($CYEAR)   $Business_sql $KPIMetrics_sql ORDER BY ValueDriver";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
    ...
    function SelectKPIMetrics($Business, $year, $KPIMetrics = '') {
         $CYEAR =  myBusinessId('CYEAR' ,$year);
         ($Business == 'All') ? $Business_sql = '' :  $Business_sql = "AND Business = '$Business'";
		if(empty($KPIMetrics)) { $KPIMetrics_sql = ''; }else{   $KPIMetrics_sql = "AND KPIMetrics = '$KPIMetrics'";		}
        $sql = "SELECT DISTINCT ValueDriver, KPIMetrics, Business FROM VW_KPIDETAILSDATA2   WHERE  ($CYEAR)  $Business_sql $KPIMetrics_sql ORDER BY ValueDriver,KPIMetrics ";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
   ...
   function SelectKPIMetrics_anomaly($Business, $year, $KPIMetrics = '') {
         $CYEAR =  myBusinessId('CYEAR' ,$year);
         ($Business == 'All') ? $Business_sql = '' :  $Business_sql = "AND Business = '$Business'";
		if(empty($KPIMetrics)) { $KPIMetrics_sql = ''; }else{   $KPIMetrics_sql = "AND KPIMetrics = '$KPIMetrics'";		}
        $sql = "SELECT DISTINCT ValueDriver, KPIMetrics, Business FROM VW_KPIDETAILSDATA2   WHERE  ($CYEAR)  $Business_sql $KPIMetrics_sql ORDER BY ValueDriver,KPIMetrics ";
    ...
    function SelectData($Business, $year , $KPIMetrics = '') {
         $CYEAR = myBusinessId('CYEAR' ,$year);
         ($Business == 'All') ? $Business_sql = '' :  $Business_sql = "AND Business = '$Business'";
		 if(empty($KPIMetrics)) { $KPIMetrics_sql = ''; }else{   $KPIMetrics_sql = "AND KPIMetrics = '$KPIMetrics'";		}
         $sql = "SELECT Business, ValueDriver,KPIMetrics,CYEAR,CPERIOD, ISNULL(KPIIndex,0) KPIIndex FROM VW_KPIDETAILSDATA2
                WHERE  ($CYEAR) $Business_sql $KPIMetrics_sql 
	...
	function SelectDataMinPeriod($Business, $year , $KPIMetrics = '') {
         $CYEAR = myBusinessId('CYEAR' ,$year);
         ($Business == 'All') ? $Business_sql = '' :  $Business_sql = "AND Business = '$Business'";
		 if(empty($KPIMetrics)) { $KPIMetrics_sql = ''; }else{   $KPIMetrics_sql = "AND KPIMetrics = '$KPIMetrics'";		}
         $sql = "SELECT DISTINCT CPERIOD FROM VW_KPIDETAILSDATA2
                WHERE  ($CYEAR) $Business_sql $KPIMetrics_sql 
                ORDER BY CPERIOD ";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
	...
	function SelectData_anomaly($Business, $year , $KPIMetrics = '') {
         $CYEAR = myBusinessId('CYEAR' ,$year);
         ($Business == 'All') ? $Business_sql = '' :  $Business_sql = "AND Business = '$Business'";
		 if(empty($KPIMetrics)) { $KPIMetrics_sql = ''; }else{   $KPIMetrics_sql = "AND KPIMetrics = '$KPIMetrics'";		}
         $sql = "SELECT Business, ValueDriver,KPIMetrics,CYEAR,CPERIOD, ISNULL(KPIIndex,0) KPIIndex FROM VW_KPIDETAILSDATA2
    ...
    function selectBusiness() {
        $sql = "SELECT DISTINCT Business FROM VW_KPIDETAILSDATA2   ORDER BY Business";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->result();
        } else {
    ...
    function selectCYEAR() {
        $sql = "SELECT DISTINCT CYEAR FROM  VW_KPIDETAILSDATA2";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->result();
        } else {
	...
	function selectChartData($Business, $ValueDriver, $KPIMetrics, $CYEAR) {
        $sql = "SELECT * FROM VW_KPIDETAILSDATA2 WHERE Business = '$Business' 
	...
	function select_period_month($q) {
        $CI = & get_instance();
        $CI->db = $this->load->database('default', true);
        $sql = "select FORMAT(CONVERT(DATETIME,LEFT('$q',4) + '-' + RIGHT('$q',2) + '-01'), 'MMM')  + ' ' + RIGHT(LEFT('$q',4),2) Period ";                 
        $query = $this->db->query($sql);
        return $query->row();
